...
def test_model(last_model, rgb_root, flow_root, split_file, num_classes, win_limit, win_stride=None):
    ...
    resume_epoch=0
    resize, labelType, model, _, __, transform, front_rear_pad, win_stride_default, target_layer = returnModel(modelName, resume_epoch, save_dir, num_classes, float(LR), input_type, frame_len, bi, trainVal=False)
    if win_stride is None: win_stride = win_stride_default
    if '512' in sampling:
        resize = (512, 512)
    ...
    sheet.append([LR, accuracy, recall, precision, F1_score, auc, ap, last_model.split('/')[-1][:-7]+f'_stride{win_stride}'])
    wb.save(resultFile)
